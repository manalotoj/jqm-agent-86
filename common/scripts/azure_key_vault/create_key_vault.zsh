...
if [[ -z "$KEY_VAULT_NAME" ]]; then
  existing_vaults=("${(@f)$(az keyvault list --resource-group "$RESOURCE_GROUP" --query '[].name' --output tsv)}")
  existing_vaults=(${existing_vaults:#})
  if (( ${#existing_vaults[@]} == 1 )); then
    KEY_VAULT_NAME="$existing_vaults[1]"
    echo "Reusing discovered Key Vault '$KEY_VAULT_NAME'."
  elif (( ${#existing_vaults[@]} > 1 )); then
